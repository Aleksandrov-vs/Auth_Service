...
async def test_user_find(make_get_request, admin_tokens, query_data, expected_answer, expected_status):
    tokens = admin_tokens
...
async def test_assign_revoke_role(make_get_request, delete_all_pg_data, admin_tokens, query_data, expected_answer,
                                  expected_status):
    ...
    # test creating role
    url = test_settings.service_url + f'/api/v1/auth/users/{query_data["user_id"]}/assign-role'
    ...
    url = test_settings.service_url + f'/api/v1/auth/users/{query_data["user_id"]}/check-permissions'
    body, status = await make_get_request(
        url,
        method='GET',
        headers={
    ...
    # revoke role
    url = test_settings.service_url + f'/api/v1/auth/users/{query_data["user_id"]}/revoke-role'
    logging.info(query_data['request'])
    body, status = await make_get_request(
        url,
        method='POST',
...
async def test_user_history(make_get_request, admin_tokens, query_data, expected_answer, expected_status):
    tokens = admin_tokens
    url = test_settings.service_url + f'/api/v1/auth/users/{query_data["user_id"]}/history'
    body, status = await make_get_request(
        url,
        method='GET',
        headers={
