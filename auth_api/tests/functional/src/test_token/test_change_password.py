...
async def test_register(make_get_request, query_data: dict, expected_answer: dict):
    ...
    if not query_data.get('headers'):
        headers = {
            "Authorization": f"Bearer {reg_body['access_token']}"
        }
    else:
    ...
    body, status = await make_get_request(
        url,
        method='POST',
        data=query_data['ch_request_body'],
        headers=headers
    )
